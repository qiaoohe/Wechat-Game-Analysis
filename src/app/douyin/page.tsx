...
import { getDouyinHomePageData } from "@/lib/services/douyin-rank-service";
import {
  BRAND_NAME,
...
export const metadata = createDouyinPageMetadata(SEO_PAGE_COPY.douyinHome);

export default async function DouyinHomePage() {
  const { latestDate, popularItems, risingItems } =
    await getDouyinHomePageData("popular");

  return (
    <div>
      <PageHeader
        title={`${DOUYIN_SITE_NAME_ZH} - ${BRAND_NAME}`}
      ...
      {!latestDate ? (
        ...
        <div className="grid gap-3 sm:gap-4 md:grid-cols-2 lg:grid-cols-3">
          <RankTablePanel
            ...
            mobileCards
            title={`${DOUYIN_RANK_TYPE_LABELS.popular} Top 10`}
            gameHrefPrefix={DOUYIN_GAMES_PATH}
            action={
              <Link
                href={`${DOUYIN_RANKINGS_PATH}?type=popular`}
          ...
          <Card className="flex h-full flex-col border-slate-200/80 bg-white md:col-span-2 lg:col-span-1">
            <CardPanelHeader
              ...
              action={
                <Link
                  href={`${DOUYIN_RISING_PATH}?type=popular`}
                  className={panelActionLinkClass}
                >
                  查看全部
                  <ArrowRight className="h-4 w-4" />
                      ...
                      >
                        <div className="flex min-w-0 flex-1 items-center gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <Link
                              href={`${DOUYIN_GAMES_PATH}/${item.gameId}`}
